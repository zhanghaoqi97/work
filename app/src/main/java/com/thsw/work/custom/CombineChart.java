...
import com.thsw.work.utils.ScreenUtils;

/**
 * 作者：chs on 2016/7/6 10:08 邮箱：657083984@qq.com 组合图表
 */
public class CombineChart extends View {
	 ...
	 */
	private Rect barRect, leftWhiteRect, rightWhiteRect, topWhiteRect,
			bottomWhiteRect;
	 ...
	 * 保存bar的左边和右边的x轴坐标点
	 */
	private List<Integer> leftPoints = new ArrayList<Integer>();
	private List<Integer> rightPoints = new ArrayList<Integer>();

	public void setOnItemBarClickListener(
			OnItemBarClickListener onRangeBarClickListener) {
	...
	private void init(Context context) {
		...
		mGestureListener = new GestureDetector(context,
				new RangeBarOnGestureListener());
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		rightWhiteRect = new Rect(screenWidth - leftMargin * 2 - 10, 0,
				screenWidth, screenHeight);
		topWhiteRect = new Rect(0, 0, screenWidth, topMargin / 2);
		bottomWhiteRect = new Rect(0, (int) yStartIndex, screenWidth,
				screenHeight);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int size = (int) ScreenUtils.dp2px(getContext(),200f);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (winds == null || mBarData == null || humidity == null
				|| temperature == null)
			return;
		// 重置3条线
		linePathW.reset();
		...
		// 画X轴 下面的和上面的
		canvas.drawLine(xStartIndex, yStartIndex, screenWidth - leftMargin,
				yStartIndex, axisPaint);
		canvas.drawLine(xStartIndex, topMargin / 2, screenWidth - leftMargin,
				topMargin / 2, axisPaint);
		// 画左边和右边的遮罩层
		...
		// 画左边的Y轴
		canvas.drawLine(xStartIndex, yStartIndex, xStartIndex, topMargin / 2,
				axisPaint);
		// 画左边的Y轴text
		drawLeftYAxis(canvas);

		// 左边Y轴的单位
		canvas.drawText(leftAxisUnit,
				xStartIndex - textPaint.measureText(leftAxisUnit) - 5,
				topMargin / 2, textPaint);

		// 画右边的Y轴
		canvas.drawLine(screenWidth - leftMargin * 2 - 10, yStartIndex,
				screenWidth - leftMargin * 2 - 10, topMargin / 2, axisPaint);
		// 画右边Y轴text
	...
	private void drawLeftYAxis(Canvas canvas) {
		...
		for (int i = 1; i <= 10; i++) {
			...
			canvas.drawLine(xStartIndex, startY, xStartIndex + 10, startY,
					axisPaint);
			String text = String.valueOf(maxDivisionValue * 0.1f * i);
			canvas.drawText(text,
					xStartIndex - textPaint.measureText(text) - 5, startY
							+ textPaint.measureText("0"), textPaint);
		}
	}

	private void drawBars(Canvas canvas) {
		for (int i = 0; i < mBarData.size(); i++) {
			barRect.left = (int) (xStartIndex + barWidth * i + barSpace
					* (i + 1) - leftMoving);
			barRect.top = (int) maxHeight
					+ topMargin
					* 2
					- (int) (maxHeight * (mBarData.get(i).getyNum() / maxValueInItems));
			...
			barRect1.top = (int) maxHeight
					+ topMargin
					* 2
					- (int) (maxHeight * (mBarData.get(i).getyNum() / maxValueInItems))
					- (int) (maxHeight * (mBarData.get(i).getyNum1() / maxValueInItems));
			barRect1.left = (int) (xStartIndex + barWidth * i + barSpace
					* (i + 1) - leftMoving);
			...
			barRect2.top = (int) maxHeight
					+ topMargin
					* 2
					- (int) (maxHeight * (mBarData.get(i).getyNum() / maxValueInItems))
					- (int) (maxHeight * (mBarData.get(i).getyNum1() / maxValueInItems))
					- (int) (maxHeight * (mBarData.get(i).getyNum2() / maxValueInItems));
			barRect2.left = (int) (xStartIndex + barWidth * i + barSpace
					* (i + 1) - leftMoving);
			...
			canvas.drawText(text, barRect.left
					- (textPaint.measureText(text) - barWidth) / 2,
					barRect.bottom + ScreenUtils.dp2px(getContext(), 10),
					textPaint);
	...
	private void drawRightYText(Canvas canvas) {
		if (rightYLabels.length == 9) {
			...
			for (int j = 0; j < 7; j++) {
				...
				// break;
				// }
				canvas.drawLine(screenWidth - leftMargin * 2 - 10, startY,
						screenWidth - leftMargin * 2 - 20, startY, axisPaint);
				String text = rightYLabels[j];
				if (j < 2) {
					textPaint.setColor(WIND_COLOR);
					canvas.drawText(text, screenWidth - leftMargin * 2 - 5,
							startY, textPaint);
				} else {
					switch (j) {
					case 2:
						canvas.drawText(text, screenWidth - leftMargin * 2 - 5,
								startY + textPaint.measureText("级"), textPaint);
						String text2 = rightYLabels[j + 1];
						textPaint.setColor(HUM_COLOR);
						canvas.drawText(text2,
								screenWidth - leftMargin * 2 - 5, startY,
								textPaint);
						break;
					case 3:
					...
					case 4:
						...
						canvas.drawText(text4,
								screenWidth - leftMargin * 2 - 5, startY
										+ textPaint.measureText("级"), textPaint);
						String text41 = rightYLabels[j + 2];
						textPaint.setColor(TEM_COLOR);
						canvas.drawText(text41, screenWidth - leftMargin * 2
								- 5, startY, textPaint);
						break;
					case 5:
	...
	private void drawCircles(Canvas canvas) {
		for (int i = 0; i < mBarData.size(); i++) {
			if (rightYLabels.length == 9) {
				...
				canvas.drawCircle(leftPoints.get(i) + barWidth / 2,
						barRect.bottom - lineHeight, RADIUS, pointPaint);
				float lineHeight2 = (humidity.get(i) - mHMinValue)
						* lineEachHeight / eachTotalValueH;
				pointPaint.setColor(HUM_COLOR);
				canvas.drawCircle(leftPoints.get(i) + barWidth / 2,
						barRect.bottom - lineHeight2 - lineEachHeight, RADIUS,
						pointPaint);
				float lineHeight3 = Math.abs(temperature.get(i) - mTMinValue)
						* lineEachHeight / eachTotalValueT;
				pointPaint.setColor(TEM_COLOR);
				canvas.drawCircle(leftPoints.get(i) + barWidth / 2,
						barRect.bottom - lineHeight3 - lineEachHeight * 2,
						RADIUS, pointPaint);
			} else {
				float lineHeight = (humidity.get(i) - mHMinValue)
						* lineEachHeightT / eachTotalValueH;
				pointPaint.setColor(HUM_COLOR);
				canvas.drawCircle(leftPoints.get(i) + barWidth / 2,
						barRect.bottom - lineHeight, RADIUS, pointPaint);
				float lineHeight1 = Math.abs(temperature.get(i) - mTMinValue)
						* lineEachHeightT / eachTotalValueT;
				pointPaint.setColor(TEM_COLOR);
				canvas.drawCircle(leftPoints.get(i) + barWidth / 2,
						barRect.bottom - lineHeight1 - lineEachHeightT, RADIUS,
						pointPaint);
			}
		}
	}
	...
	private void drawLines(int i) {
		if (rightYLabels.length == 9) {
			float lineHeight = winds.get(i) * lineEachHeight / 10f;
			if (i == 0) {
				linePathW.moveTo(barRect.left + barWidth / 2, barRect.bottom
						- lineHeight);
			} else {
				linePathW.lineTo(barRect.left + barWidth / 2, barRect.bottom
						- lineHeight);
			}

			float lineHeight2 = (humidity.get(i) - mHMinValue) * lineEachHeight
					/ eachTotalValueH;
			if (i == 0) {
				linePathH.moveTo(barRect.left + barWidth / 2, barRect.bottom
						- lineHeight2 - lineEachHeight);
			} else {
				linePathH.lineTo(barRect.left + barWidth / 2, barRect.bottom
						- lineHeight2 - lineEachHeight);
			}
			...
			float lineHeight3 = Math.abs(temperature.get(i) - mTMinValue)
					* lineEachHeight / eachTotalValueT;
			if (i == 0) {
				linePathT.moveTo(barRect.left + barWidth / 2, barRect.bottom
						- lineHeight3 - lineEachHeight * 2);
			} else {
				linePathT.lineTo(barRect.left + barWidth / 2, barRect.bottom
						- lineHeight3 - lineEachHeight * 2);
			}
		} else {
			float lineHeight2 = (humidity.get(i) - mHMinValue)
	 ...
	 *            温度线形图的值
	 */
	public void setItems(List<BarChartBean> items, List<Float> winds,
			List<Float> humidity, List<Float> temperature, String[] rightYLabels) {
		...
		// 计算最大值
		maxValueInItems = items.get(0).getyNum() + items.get(0).getyNum1()
				+ items.get(0).getyNum2();
		for (BarChartBean barChartBean : items) {
			float totalNum = barChartBean.getyNum() + barChartBean.getyNum1()
					+ barChartBean.getyNum2();
	...
	private void changeRightYLabels() {
		...
		int hMax = (int) (getRangeTop(unHMaxScaleValue) * Math.pow(10,
				hMaxScale));
		int hMin = (int) (getRangeMin(unHMinScaleValue) * Math.pow(10,
				hMinScale));
		...
		int tMax = (int) (getRangeTop(Math.abs(unTMaxScaleValue)) * Math.pow(
				10, tMaxScale));
		int tMin = (int) (getRangeMin(Math.abs(unTMinScaleValue)) * Math.pow(
				10, tMinScale));
		...
		if (rightYLabels.length == 9) {
			...
			rightYLabels[7] = tMin + (tMax - tMin) / 2
					+ getResources().getString(R.string.degree_centigrade);
			rightYLabels[8] = tMax
					+ getResources().getString(R.string.degree_centigrade);
		} else {
			...
			rightYLabels[3] = tMin
					+ getResources().getString(R.string.degree_centigrade);
			rightYLabels[4] = tMin + (tMax - tMin) / 2
					+ getResources().getString(R.string.degree_centigrade);
			rightYLabels[5] = tMax
					+ getResources().getString(R.string.degree_centigrade);
		}
	...
	private void getRange(float maxValueInItems, float min) {
		...
		maxDivisionValue = (float) (getRangeTop(unScaleValue) * Math.pow(10,
				scale));
	 ...
	 * @author A Shuai
	 */
	private class RangeBarOnGestureListener implements
